...
import { ChevronLeft, Maximize2, Tag, Package, Box, Zap, Trash2, Sofa, PlusCircle, PenTool, LayoutGrid, CheckCircle2, AlertCircle, Users, CreditCard, User, Building } from 'lucide-react-native';
    ...
    <View className="flex-1 bg-[#f4f7f9]">
      ...
      <View className="bg-white pt-14 pb-4 px-5 border-b border-slate-200 shadow-sm z-10 relative">
        ...
        <View className="z-10 flex-row justify-between items-end">
          ...
          <View className="bg-blue-50 px-3 py-1 rounded-full border border-blue-100 flex-row items-center mb-1 max-w-[50%] flex-shrink">
             {/* @ts-ignore */}
             <Tag size={10} color="#2563eb" />
             <Text className="text-blue-700 font-bold text-[10px] ml-1.5 uppercase tracking-widest flex-shrink" numberOfLines={1}>{industrySector}</Text>
          </View>
        </View>
      </View>
        ...
        <View className="flex-row gap-3 mb-3">
          ...
          <View className="bg-white rounded-[16px] p-2.5 flex-1 shadow-sm border border-slate-200 flex-row items-center">
            ...
            <View className="flex-1">
              ...
              <Text className={`font-black text-[12px] ${isPaid ? 'text-green-700' : 'text-orange-600'}`} numberOfLines={1}>{paymentStatusText}</Text>
            </View>
          </View>
        </View>
