  ...
  return (
    <View className="flex-1 bg-[#f4f7f9]">
      <ScrollView className="flex-1" showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 100 }}>
        ...
        <View className="px-5 -mt-16 z-20">

          {/* 2. Event Countdown Timer */}
          <EventTimer targetDate="2026-08-21T09:00:00" />

          {/* 3. My Digital Badge - Lanyard Style Exhibition Badge */}
          {/* 3. My Digital Pass */}
          <View className="items-center mb-4 mt-1">
            <View className="w-full bg-white rounded-[24px] shadow-lg border border-slate-200 overflow-hidden relative">
              ...
              <View className="px-5 py-4 items-center">
                ...
                <View className="bg-blue-50 px-4 py-1 rounded-full mb-4 border border-blue-100">
                  <Text className="text-blue-700 font-bold text-[10px] uppercase tracking-widest">{sector}</Text>
                </View>

                {/* Info Row */}
                <View className="flex-row items-center w-full bg-slate-50 p-3 rounded-[16px] border border-slate-100 mb-4">
          ...
          {/* (Moved Event Countdown Timer above badge) */}
          ...
          {/* 5. My Event Checklist */}
          <SectionHeader title="My Event Checklist" actionText="View All Tasks →" />
          ...
          {/* 6. My Bookings & Services */}
          <SectionHeader title="My Bookings & Services" actionText="View All →" />
          ...
          <SectionHeader title="Important Announcements" actionText="View All" />
          ...
          {/* 8. Event Documents */}
          <SectionHeader title="Event Documents" actionText="View All" />
          ...
          {/* 9. Venue Map Redirect */}
          <Text className="text-[#1a3a7c] font-black text-[17px] tracking-tight mb-2 ml-1">Venue Location</Text>
          <TouchableOpacity
...
function EventTimer({ targetDate }: { targetDate: string }) {
  ...
  return (
    <View className="bg-white rounded-[20px] py-3 px-4 shadow-sm border border-slate-200 mb-2 mx-1 relative overflow-hidden">
      <View className="absolute right-[-20px] top-[-20px] w-24 h-24 rounded-full border-[8px] border-blue-50/50" />
      <Text className="text-slate-400 font-bold text-[9px] uppercase tracking-widest text-center mb-2">Event Starts In</Text>
...
function TimeUnit({ value, label }: { value: number, label: string }) {
  return (
    <View className="items-center">
      <View className="bg-blue-50 w-11 h-11 rounded-xl items-center justify-center border border-blue-100 mb-1 shadow-sm">
        <Text className="text-blue-700 font-black text-[18px]">{value < 10 ? `0${value}` : value}</Text>
      </View>
      <Text className="text-slate-500 font-bold text-[9px] uppercase tracking-wider">{label}</Text>
    </View>
  );
}
...
function InfoCard({ title, value, subtitle, icon }: { title: string, value: string, subtitle: string, icon: any }) {
  return (
    <View className="bg-white w-[48%] rounded-[20px] p-3 shadow-sm border border-slate-100 mb-2">
      <View className="w-8 h-8 bg-[#eff6ff] rounded-full items-center justify-center mb-2">
        {icon}
      </View>
      <Text className="text-slate-400 font-bold uppercase text-[9px] tracking-widest mb-1">{title}</Text>
      <Text className="text-[#0f172a] font-black text-[18px] mb-0.5">{value}</Text>
      <Text className="text-slate-500 font-bold text-[11px]">{subtitle}</Text>
    </View>
  );
}
...
function ChecklistItem({ icon, title, desc, status, isLast = false }: { icon: string, title: string, desc: string, status: string, isLast?: boolean }) {
  ...
  return (
    <TouchableOpacity className={`flex-row p-4 items-center ${!isLast ? 'border-b border-slate-100' : ''}`}>
...
function ServiceCard({ icon, title, status }: { icon: string, title: string, status: string }) {
  ...
  return (
    <View className="bg-white rounded-[20px] p-3 shadow-sm border border-slate-100 mr-2 w-[130px]">
...
function DocumentCard({ title, size, isLast = false }: { title: string, size: string, isLast?: boolean }) {
  return (
    <TouchableOpacity className={`bg-white p-3 flex-row items-center shadow-sm border border-slate-200 ${isLast ? 'rounded-[20px]' : 'rounded-[20px] mb-2'}`}>
